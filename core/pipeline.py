...
async def run(ctx: RunContext, emit: Emit = None) -> dict:
    ...
    6. Rank candidates
    7. Scrape (standard/deep only, cheap skips)
    ...
    Returns:
        v2-identical ResearchResponse dict
    """
    ...
    for topic_name in ctx.topics:
        ...
        # ═══════════════════════════════════════════════
        # Stage 7: Scrape (standard/deep only)
        # ═══════════════════════════════════════════════
        if ctx.policy.depth != "cheap" and ctx.policy.max_extraction_chars > 0:
            _emit(StageStart(stage="scrape", count=len(candidates)))
