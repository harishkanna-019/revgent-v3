...
- Pure tests verify stage wiring, event emission, budget handling.
- Real API tests run full cheap-depth pipeline against OpenRouter + SearXNG.
"""
...
# ═══════════════════════════════════════════════
# Real API tests (end-to-end cheap depth)
# ═══════════════════════════════════════════════
...
class TestPipelineReal:
    ...
    async def test_standard_depth_branch(self):
        """Standard depth uses LLM topic analysis and query generation."""
        ctx = _make_ctx(depth="standard")
        response = await run(ctx)

        # Should have consumed tokens for topic + queries + validation
